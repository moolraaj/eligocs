...
function ServicesSlugFaq({ ele }) {
    ...
    return (
        <>
            <div className="services_faq_section_first">
                ...
                <div className="services_inner_page_faq_wrapper">
                    {Array.isArray(ele.acf?.services_inner_faq_section) &&
                        ele.acf.services_inner_faq_section.map((e, index) => (
                            e.faq_tittle && e.faq_description ? ( // Check if both faq_tittle and faq_description have content
                                <div key={index} className="inner_service_faq">
                                    <div
                                        onClick={() => toggleAccordion(index)}
                                        className={`services_inner_question arrow-icon ${
                                    ...
                                    {activeIndex === index && (
                                        <div className="services_inner_answer">
                                            <span
                                                dangerouslySetInnerHTML={{ __html: e.faq_description }}
                                            ></span>
                                        </div>
                                    )}
                                </div>
            ...
            <div className="services_faq_section_second">
                <p id="faq_second_title_one">{ele.acf.faq_second_headong_one || ""}</p>
                <h2 id="faq_second_title_two">{ele.acf.faq_second_heading_two || ""}</h2>
                <div className="services_inner_page_faq_wrapper">
                    {Array.isArray(ele.acf?.services_inner_faq_section_second) &&
                        ele.acf.services_inner_faq_section_second.map((e, index) => (
                            e.faq_tittle && e.faq_description ? ( // Check if both faq_tittle and faq_description have content
                                <div key={index} className="inner_service_faq">
                                    <div
                                        onClick={() => toggleAccordionSecond(index)}
                                        className={`services_inner_question arrow-icon ${
                                            activeIndexSecond === index ? 'open' : 'closed'
                                        }`}
                                    >
                                        <h2>{e.faq_tittle}</h2>
                                        <div
